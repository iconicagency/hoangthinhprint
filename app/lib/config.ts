export const homeConfig = {
  hero: {
    title: "Giải pháp bao bì toàn diện cho doanh nghiệp",
    subtitle: "Thiết kế sáng tạo - In ấn chất lượng - Giao hàng đúng hẹn. Đối tác tin cậy của hơn 500+ thương hiệu.",
    tagline: "GIẢI PHÁP IN ẤN BAO BÌ TRỌN GÓI",
    slides: [
      "https://picsum.photos/seed/printingpress/1920/1080",
      "https://picsum.photos/seed/factory2/1920/1080",
      "https://picsum.photos/seed/design3/1920/1080",
    ],
    benefits: [
      "Thiết kế 3D miễn phí",
      "In mẫu test màu",
      "Giao hàng tận nơi",
      "Giá gốc tại xưởng"
    ]
  },
  stats: [
    { number: "17", label: "Năm kinh nghiệm", suffix: "+" },
    { number: "500", label: "Đối tác tin tưởng", suffix: "+" },
    { number: "300", label: "Đơn hàng lớn nhất", suffix: "tr" },
    { number: "99", label: "Giao đúng hẹn", suffix: "%" },
  ],
  partners: [
    "Vingroup",
    "Viettel",
    "FPT",
    "Samsung",
    "BIDV"
  ],
  videoSection: {
    title: 'Tham Quan Xưởng In Hoàng Thịnh',
    description: 'Khám phá quy trình sản xuất khép kín từ thiết kế, in ấn đến gia công thành phẩm tại xưởng in quy mô lớn của chúng tôi.',
    videoUrl: '#'
  }
};
