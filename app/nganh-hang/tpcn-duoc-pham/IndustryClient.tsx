...
export default function IndustryClient() {
  ...
  const defaultFaqs = [
    { question: 'MOQ cho hộp cứng TPCN là bao nhiêu?', answer: 'Chúng tôi nhận sản xuất từ 500 hộp trở lên.' },
    { question: 'Có ép kim vàng tại xưởng không?', answer: 'Có, In Hoàng Thịnh sở hữu hệ thống máy ép kim hiện đại ngay tại xưởng.' },
    { question: 'Thời gian sản xuất bao lâu?', answer: 'Từ 7-10 ngày làm việc kể từ khi chốt thiết kế.' },
    { question: 'Sai màu thì sao?', answer: 'Cam kết in đúng màu theo chuẩn Pantone. Nếu sai màu, xưởng sẽ in lại hoàn toàn miễn phí.' },
  ];
  ...
  const productsList = acf.productsList && acf.productsList.length > 0 ? acf.productsList : [
    { title: 'Hộp cứng âm dương', description: 'Phổ biến nhất cho TPCN. Ép kim vàng, lót nhung.' },
    { title: 'Hộp cứng nam châm', description: 'Sang trọng, đóng mở dễ dàng. Cho dòng sản phẩm premium.' },
    { title: 'Hộp giấy in offset', description: 'Cho sản phẩm đóng hộp nhỏ, dược phẩm đơn vị.' },
  ];
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-main)] font-sans">
      <section className="bg-slate-900 text-white py-20 px-8 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-serif font-bold mb-4 tracking-tight">{acf.heroTitle || 'Bao Bì Cho Ngành TPCN & Dược Phẩm'}</h1>
          <p className="text-xl text-slate-300 max-w-2xl">{acf.heroSubtitle || 'Hộp cứng ép kim vàng, chuẩn Pantone — Nâng tầm giá trị sản phẩm TPCN'}</p>
        </div>
      </section>

      <section className="py-24 px-8 bg-[var(--bg)] text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-[var(--text-main)] mb-6">{acf.introTitle || 'Bao Bì Cho Ngành TPCN & Dược Phẩm'}</h2>
          ...
          {acf.introContent ? (
            ...
            <p className="text-[var(--text-dim)] text-lg leading-relaxed">Ngành TPCN & Dược phẩm yêu cầu bao bì vừa cao cấp vừa chuyên nghiệp.</p>
          )}
        </div>
      </section>
      ...
      <section className="py-24 px-8 bg-[var(--card-bg)] border-y border-[var(--border)]">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-[var(--text-main)] mb-12 text-center">{acf.whyTitle || 'Tại sao TPCN cần bao bì cao cấp?'}</h2>
      ...
      <section className="py-24 px-8 bg-[var(--bg)]">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-[var(--accent)] font-bold text-lg">{acf.pricingText || 'Giá từ 10.200đ/hộp (MOQ 500). Đơn từ 5.000 hộp giảm thêm 10-15%.'}</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {acf.sampleImages?.nodes?.length > 0
              ...
              : [1, 2, 3, 4].map((i) => (
                <div key={i} className="relative aspect-square rounded-2xl overflow-hidden border border-[var(--border)] group">
                  <Image src={`https://picsum.photos/seed/tpcn${i}/400/400`} alt={`Mẫu ${i}`} fill className="object-cover group-hover:scale-110 transition-transform duration-500" referrerPolicy="no-referrer" />
                </div>
              ))}
          </div>
        </div>
