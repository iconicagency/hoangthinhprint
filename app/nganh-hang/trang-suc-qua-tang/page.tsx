...
export default function JewelryPage() {
  ...
  const defaultFaqs = [
    ...
    { question: 'Co in logo ep kim khong?', answer: 'Co, ep kim logo (vang, bac, rose gold) la the manh cua chung toi.' },
  ];
  ...
  const whyList = acf.whyList && acf.whyList.length > 0 ? acf.whyList.map((w: any) => w.item) : [
    'Kich thuoc nho — can bao bi tinh xao',
    'Nam cham dong mo em — tao trai nghiem premium',
    'Lot nhung giu co dinh san pham',
    'Ep kim logo tao nhan dien thuong hieu',
    'Thiet ke mien phi — duyet 3D truoc khi san xuat'
  ];
  const productsList = acf.productsList && acf.productsList.length > 0 ? acf.productsList : [
    { title: 'Hop cung nam cham nho', description: 'Kich thuoc vua nhan, day chuyen, bong tai.' },
    { title: 'Hop cung bao diem', description: 'Keo ra day vao, sang trong.' },
    { title: 'Tui giay mini', description: 'Tui giay nho di kem. Hoan thien set qua.' }
  ];
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text-main)] font-sans">
      <section className="bg-slate-900 text-white py-20 px-8 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="text-sm text-slate-400 mb-6 flex items-center gap-2">
            ...
            <span>/</span><span className="text-white">Trang Suc & Qua Tang</span>
          </div>
      ...
      <section className="py-24 px-8 bg-[var(--bg)] text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-[var(--text-main)] mb-6">{acf.introTitle || 'Bao Bi Cho Nganh Trang Suc & Qua Tang'}</h2>
          ...
          {acf.introContent ? (
            ...
            <p className="text-[var(--text-dim)] text-lg leading-relaxed">Trang suc va qua tang can bao bi nho gon nhung tinh te.</p>
          )}
        </div>
      </section>
      <section className="py-24 px-8 bg-[var(--card-bg)] border-y border-[var(--border)]">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-[var(--text-main)] mb-12 text-center">{acf.whyTitle || 'Tai sao trang suc can hop rieng biet?'}</h2>
      ...
      <section className="py-24 px-8 bg-[var(--bg)]">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-[var(--accent)] font-bold text-lg">{acf.pricingText || 'Hop nho tu 10.200d. MOQ 500.'}</p>
        </div>
      </section>
      <section className="py-24 px-8 bg-[var(--card-bg)] border-y border-[var(--border)]">
        <div className="max-w-6xl mx-auto">
