  ...
  const handleLogin = async () => {
    try {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        setError('Tên miền này chưa được cấp quyền. Vui lòng thêm tên miền Vercel của bạn vào danh sách "Authorized domains" trong Firebase Console (Authentication -> Settings -> Authorized domains).');
      } else {
        ...
        <div className="bg-[var(--card-bg)] p-8 md:p-10 rounded-3xl border border-[var(--border)] shadow-sm">
          ...
          {!user ? (
            <div className="text-center py-12">
              ...
              <button 
                onClick={handleLogin}
                className="bg-[var(--accent)] text-[var(--bg)] font-bold py-3 px-8 rounded-lg hover:opacity-90 transition-opacity"
              >
